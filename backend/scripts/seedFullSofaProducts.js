...
function normalizeProduct(raw) {
  ...
  if (!p.description) {
    const bits = [
      'Premium sofa from THECASAWOOD.',
      p.upholsteryMaterial ? `Upholstery: ${p.upholsteryMaterial}.` : null,
      p.material ? `Material: ${p.material}.` : null,
      p.deliveryCondition ? `Delivery: ${p.deliveryCondition}.` : null
    ].filter(Boolean);
...
async function main() {
  ...
  const jsonPath = path.resolve(__dirname, '../models/full_sofa_products.json');
  const contents = await fs.readFile(jsonPath, 'utf-8');
  const data = JSON.parse(contents);

  if (!Array.isArray(data)) {
    throw new Error('Expected JSON array in full_sofa_products.json');
  }
  ...
  for (const raw of data) {
    ...
    // Idempotent: treat (name + category + color) as stable key
    const filter = {
      name: product.name,
