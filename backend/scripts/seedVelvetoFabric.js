...
/**
 * Seed Fabric collection from velveto_colors.json.
 * JSON format: { "code": "VELVETO 901", "hex": "#E8E5DC", "approx": "Ivory" }
 * Fabric model: { name: "VELVETO", colors: [ { code: "901", name: "Ivory", color: "#E8E5DC" } ] }
 */
async function main() {
  ...
  const jsonPath = path.resolve(__dirname, '../../frontend/velveto_colors.json');
  const contents = await fs.readFile(jsonPath, 'utf-8');
  const rows = JSON.parse(contents);

  if (!Array.isArray(rows)) {
    throw new Error('Expected JSON array in velveto_colors.json');
  }

  await mongoose.connect(uri);

  // Group by fabric name (e.g. "VELVETO")
  const byFabric = {};
  for (const row of rows) {
    ...
    const fabricName = parts[0] || 'VELVETO';
  ...
  console.log('Velveto fabric seed complete.');
}
