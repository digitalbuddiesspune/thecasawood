...
/**
 * Seed Fabric collection from aboone_colors.json.
 * JSON format: { "code": "ABOONE 101", "hex": "#C9C1B6", "approx": "Light Greige" }
 * Fabric model: { name: "ABOONE", colors: [ { code: "101", name: "Light Greige", color: "#C9C1B6" } ] }
 */
async function main() {
  ...
  const jsonPath = path.resolve(__dirname, '../aboone_colors.json');
  const contents = await fs.readFile(jsonPath, 'utf-8');
  const rows = JSON.parse(contents);

  if (!Array.isArray(rows)) {
    throw new Error('Expected JSON array in aboone_colors.json');
  }

  await mongoose.connect(uri);

  // Group by fabric name (e.g. "ABOONE")
  const byFabric = {};
  for (const row of rows) {
    ...
    const fabricName = parts[0] || 'ABOONE';
  ...
  console.log('Aboone fabric seed complete.');
}
